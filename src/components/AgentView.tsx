...
export default function AgentView({ crewId }: AgentViewProps) {
  ...
  const loadAgents = async () => {
    try {
      ...
      const endpoint = crewId ? API_ENDPOINTS.agentsByCrew(crewId) : API_ENDPOINTS.agents;
  ...
  if (!crewId) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-lg font-medium text-gray-900 mb-2">Keine Crew ausgewählt</h3>
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Verwalte die KI-Agents für Crew #{crewId}</p>
        </div>
          ...
          >
            <div className="flex items-start justify-between mb-4">
              <div className="w-12 h-12 bg-purple-600 rounded-lg flex items-center justify-center">
                <span className="text-white text-lg">{getRoleIcon(agent.role)}</span>
              </div>
              <span
                className={`px-3 py-1 rounded-full text-xs font-medium ${getRoleColor(agent.role)}`}
              >
                {agent.role}
              </span>
            </div>
        ...
        {agents.length === 0 && (
          <div className="col-span-full">
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">Keine Agents vorhanden</h3>
              <p className="text-gray-600 mb-6">Erstelle den ersten Agent für diese Crew</p>
